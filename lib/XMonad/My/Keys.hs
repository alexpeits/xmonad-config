...
import XMonad hiding (keys, terminal, workspaces)
  ...
  [ ((modMask .|. shiftMask, xK_Return), spawn $ X.terminal conf),
    ...
    ++ let view =
        ...
        in [ ((m .|. modMask, k), windows (f i))
             ...
             if hasMediaKeys
               then
                 [ ((modMask .|. controlMask, xK_F2), spawn $ volumeDown +++ volumeShow),
                 ...
                 [ ((modMask, xK_F2), spawn "amixer -D pulse set Master 3%-"),
